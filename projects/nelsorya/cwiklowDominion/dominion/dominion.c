...
int handCard(int handPos, struct gameState *state) {
  ...
  if(handPos >= 0 && handPos < state->handCount[currentPlayer]) 
  {
     if(state->hand[currentPlayer][handPos] >= 0 && state->hand[currentPlayer][handPos] <= 26) 
...
int whoseTurn(struct gameState *state) {
   if(state->whoseTurn < 0)
      return -9999;
   if(state->whoseTurn <= state->numPlayers)
...
int isGameOver(struct gameState *state) {
  ...
  if (state->supplyCount[province] < 1)
    {
      return 1;
    }
...
int playAdventurer(int currentPlayer, struct gameState *state, int handPos)
{
	...
	int drawntreasure = 0;
        ...
        //get number of treasure cards in deck and discard piles
        int i, iterations, counter = 0;
	...
	while(drawntreasure<iterations)
	{
