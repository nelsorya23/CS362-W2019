...
*
*Tests:
*This function is tested when the village card is played when in the
*player's hand position of 0, 4, and 3.
*
*The objective of the test is to see if the players hand count stays the  
*same and the player's action count is 2 after the card is played and 
*ensure the card was discarded to played pile.
* Include in makefile:
...
void runTests(struct gameState* G, struct gameState* testG, int* flag, int handPos, int currentPlayer, int expectedHandCount, int expectedActionCount )
{
   ...
   //test if the number of actions is 2
   testResult(testG->numActions, expectedActionCount, flag, "Is the number of actions 2");

   //test if card was discarded and is now in the played pile
   testResult(testG->playedCards[0], village, flag, "Is the card played in the played pile"); 
...
int main()
{
   ...
   int expectedActionCount = 2; //number of actions left after gaining 2 actions
   ...
   copyG.hand[currentPlayer][4] = village;
   ...
   printf("\nRunning Test 1: Check if number of cards in player's hand stays the same with village card in hand position 4\n");
   ...
   expectedActionCount = 2; //number of actions left after gaining 2 actions
   copyG.hand[currentPlayer][0] = village;
   copyG.hand[currentPlayer][1] = estate;
   copyG.hand[currentPlayer][2] = estate;
   copyG.hand[currentPlayer][3] = estate;
   ...
   printf("\nRunning Test 2: Check if number of cards in player's hand stays the same with village card in hand position 0\n");
   runTests(&copyG, &testG, &flag, handPos, currentPlayer, expectedHandCount, expectedActionCount);
   ...
   printf("\nRunning Test 3: Check if number cards in player's hand stays the same with village card in hand position 3\n");
   runTests(&copyG, &testG, &flag, handPos, currentPlayer, expectedHandCount, expectedActionCount);
