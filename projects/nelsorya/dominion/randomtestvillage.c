...
void runTest(struct gameState* G, struct gameState* testG, struct testValues* tv)
{
   ...
   testResult(testG->playedCards[testG->playedCardCount - 1], village, &flag, "Is village card in played card pile");
...
void setGame(struct gameState* G, int currentPlayer, struct testValues* tv)
{
   ...
   //make sure village card is in player's hand at a random place
   int randMax;
   ...
   G->discardCount[currentPlayer] = randomNum(1, MAX_DECK);
